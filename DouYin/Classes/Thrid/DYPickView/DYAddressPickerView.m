...
#import "DYAddressPickerView.h"
#import "DYAddressModel.h"
#import "YYModel.h"

@interface DYAddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
// 选中后的回调
@property (nonatomic, copy) DYAddressResultBlock resultBlock;

@end

@implementation DYAddressPickerView
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(DYAddressResultBlock)resultBlock {
    DYAddressPickerView *addressPickerView = [[DYAddressPickerView alloc] initWithDefaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect resultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}

#pragma mark - 初始化地址选择器
- (instancetype)initWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(DYAddressResultBlock)resultBlock {
    if (self = [super init]) {
        // 默认选中
...
- (void)loadData {
    ...
    for (NSDictionary *dic in arrData) {
        // 此处用 YYModel 进行解析
        DYProvinceModel *proviceModel = [DYProvinceModel yy_modelWithDictionary:dic];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    DYProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
    DYCityModel *cityModel = provinceModel.city[rowOfCity];
    if (component == 0) {
        //返回省个数
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {//省
        DYProvinceModel *provinceModel = self.addressModelArr[row];
        showTitleValue = provinceModel.name;
    }
    if (component == 1) {//市
        DYProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        DYCityModel *cityModel = provinceModel.city[row];
        showTitleValue = cityModel.name;
    }
    if (component == 2) {//区
        DYProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        DYCityModel *cityModel = provinceModel.city[rowOfCity];
        DYTownModel *townModel = cityModel.town[row];
    ...
    if (rowOfProvince < self.addressModelArr.count) {
        ...
        if (rowOfCity < provinceModel.city.count) {
            ...
            if (rowOfTown < cityModel.town.count) {
                DYTownModel *townModel = cityModel.town[rowOfTown];
                arr = @[provinceModel, cityModel, townModel];
            }
        }
    }
    return arr;
...
- (void)scrollToRow:(NSInteger)firstRow secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow {
    if (firstRow < self.addressModelArr.count) {
        ...
        DYProvinceModel *provinceModel = self.addressModelArr[firstRow];
        if (secondRow < provinceModel.city.count) {
            rowOfCity = secondRow;
            [self.pickerView reloadComponent:1];
            DYCityModel *cityModel = provinceModel.city[secondRow];
