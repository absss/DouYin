...
int const CSFSegmentHeight = 44;//可根据项目需求设置高度

@interface CSFSegmentView ()
...
- (void)segmentBasicSetting {
    ...
    _segmentHeight = CSFSegmentHeight;
    _minItemSpace = 40.;
...
- (void)didClickButton:(UIButton *)button {
    ...
    if (self.shouldSelectedBlock) {
       should = self.shouldSelectedBlock(button, button.tag);
    }
    if (!should) {
        return;
    }
    ...
    if (self.didSelectedBlock) {
        self.didSelectedBlock(button,_selectedButton.tag);
    }
}
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (_style == CSFSegmentStyleDefault) {
...
- (void)adjustOffsetXToFixIndicatePosition:(CGFloat)offsetX {
    ...
    CGFloat originMovedX = _style == CSFSegmentStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离
    
    CGFloat targetButtonWidth = _style == CSFSegmentStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == CSFSegmentStyleDefault? ([self widthAtIndex:[self index]] - 2 * _buttonSpace) : [self widthAtIndex:[self index]];
...
- (void)setStyle:(CSFSegmentStyle)style {
    _style = style;
    
    if (style==CSFSegmentStyleFlush) {
