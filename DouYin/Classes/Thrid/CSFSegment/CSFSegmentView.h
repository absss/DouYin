...
typedef NS_ENUM(NSUInteger, CSFSegmentStyle) {
    CSFSegmentStyleDefault,    //  指示杆和按钮的标题齐平
    CSFSegmentStyleFlush,      //  指示杆和按钮宽度齐平
};
NS_ASSUME_NONNULL_BEGIN
FOUNDATION_EXPORT int const CSFSegmentHeight;

@interface CSFSegmentView : UIView

@property (nonatomic) CSFSegmentStyle style;
...
@property (nonatomic, copy)   BOOL(^shouldSelectedBlock)(UIButton *button,NSUInteger index);
@property (nonatomic, copy)   void(^didSelectedBlock)(UIButton *button,NSUInteger index);
